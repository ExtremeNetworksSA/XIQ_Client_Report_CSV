...
def check_for_file():
    ...
    for filename in os.listdir(filePath):
        ...
        if filename.endswith('.zip'):
            ...
            filePath, csv_files = check_for_file()
            return filePath, csv_files
...
def checkArchives():
    archivePath = '{}/Archive'.format(PATH)
    list_of_files = os.listdir(archivePath)

    while len(list_of_files) > 1:
        full_path = ["{}/{}".format(archivePath,x) for x in list_of_files]
        oldest_file = min(full_path, key=os.path.getctime)
        os.remove("{}".format(oldest_file))
        list_of_files = os.listdir(archivePath)
...
filePath, csv_files = check_for_file()
outputPath = '{}/OutputFile'.format(PATH)
archivePath = '{}/Archive'.format(PATH)
...
for filename in csv_files:
    ...
    fullFileName = '{}/{}'.format(filePath,filename)
    client_list = csv_import(fullFileName)
    ...
    excelname += '.xlsx'
    workbook = xlsxwriter.Workbook('{}/{}'.format(outputPath, excelname))
    ...
    worksheet.merge_range('A2:A7','',Label_format)
    ...
    shutil.move(fullFileName, '{}/{}'.format(archivePath, filename))
